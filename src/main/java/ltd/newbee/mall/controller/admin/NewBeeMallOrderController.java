...
public class NewBeeMallOrderController {
    ...
    @Resource
    NewBeeMallOrderService newBeeMallOrderService;
    ...
    public Result list(@RequestParam Map<String,Object> params){
        if (StringUtils.isEmpty(params.get("page"))||StringUtils.isEmpty(params.get("limit"))){
            return ResultGenerator.genFailResult("list的参数异常");
        }
    ...
    public Result checkDone(@RequestBody Long[] ids){
        if(ids.length<1) {
        return ResultGenerator.genFailResult("ids传入失败");
        }
...
if (ids.length<1){
    return ResultGenerator.genFailResult("ids传参除了问题");
}
